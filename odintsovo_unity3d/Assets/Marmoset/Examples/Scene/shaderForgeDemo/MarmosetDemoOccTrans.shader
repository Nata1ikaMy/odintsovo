...
/////// Normals:
                float2 node_153 = i.uv0;
                float3 normalLocal = UnpackNormal(tex2D(_BumpMap,TRANSFORM_TEX(node_153.rg, _BumpMap))).rgb;
...
/////// Diffuse:
                ...
                #endif
///////// Gloss:
                float4 node_42 = tex2D(_SpecTex,TRANSFORM_TEX(node_153.rg, _SpecTex));
...
////// Specular:
                ...
                finalColor += diffuseLight * ((tex2D(_MainTex,TRANSFORM_TEX(node_153.rg, _MainTex)).rgb*_DiffuseColor.rgb)*_DiffuseColor.a);
                finalColor += specular;
/// Final Color:
...
/////// Normals:
                ...
                float3 normalLocal = UnpackNormal(tex2D(_BumpMap,TRANSFORM_TEX(node_154.rg, _BumpMap))).rgb;
                float3 normalDirection =  normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
...
/////// Diffuse:
                ...
                float3 diffuse = max( 0.0, NdotL) * attenColor;
///////// Gloss:
                float4 node_42 = tex2D(_SpecTex,TRANSFORM_TEX(node_154.rg, _SpecTex));
                float node_56 = (node_42.a*_SpecularColor.a*_SpecularSharpness);
                float gloss = node_56;
                float specPow = exp2( gloss * 10.0+1.0);
////// Specular:
                ...
                float3 finalColor = 0;
                float3 diffuseLight = diffuse;
                finalColor += diffuseLight * ((tex2D(_MainTex,TRANSFORM_TEX(node_154.rg, _MainTex)).rgb*_DiffuseColor.rgb)*_DiffuseColor.a);
                finalColor += specular;
/// Final Color:
